 ...
 */
export function useProfileToolsWithIds(mcpGatewayId: string | undefined) {
  return useQuery({
    queryKey: ["mcp-gateways", mcpGatewayId, "tools", "mcp-only"],
    queryFn: async () => {
      if (!mcpGatewayId) return [];
