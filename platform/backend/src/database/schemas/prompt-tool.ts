...
/**
 * Junction table for assigning tools directly to prompts (agents).
 * Used for A2A and Chat tool execution, separate from MCP Gateway assignments.
 *
 * Tools can be assigned to EITHER:
 * 1. MCP Gateway - for external agents connecting via MCP protocol
 * 2. Prompt (Agent) - for internal A2A and Chat tool execution
 *
 * Both assignment types coexist independently.
 */
const promptToolsTable = pgTable(
  "prompt_tools",
  {
  ...
  (table) => [unique().on(table.promptId, table.toolId)],
);

export default promptToolsTable;
