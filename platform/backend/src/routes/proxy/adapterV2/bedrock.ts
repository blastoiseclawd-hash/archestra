/**
 * Bedrock LLM Proxy Adapter - OpenAI-compatible
 *
 * Bedrock uses an OpenAI-compatible API at https://bedrock-mantle.us-east-1.api.aws/v1
 * This adapter reuses OpenAI's logic with Bedrock-specific configuration.
 *
 * @see https://inference-docs.bedrock.ai/
 */
...
type BedrockRequest = Bedrock.Types.ChatCompletionsRequest;
type BedrockResponse = Bedrock.Types.ChatCompletionsResponse;
type BedrockMessages = Bedrock.Types.ChatCompletionsRequest["messages"];
type BedrockHeaders = Bedrock.Types.ChatCompletionsHeaders;
type BedrockStreamChunk = Bedrock.Types.ChatCompletionChunk;

type BedrockToolResultImageBlock = {
  type: "image_url";
  image_url: {
    url: string;
...
type BedrockToolResultTextBlock = {
  type: "text";
  text: string;
};

type BedrockToolResultContentBlock =
  | BedrockToolResultImageBlock
  | BedrockToolResultTextBlock;

type BedrockToolResultContent = string | BedrockToolResultContentBlock[];
  ...
  implements LLMRequestAdapter<BedrockRequest, BedrockMessages>
{
  ...
  convertToolResultContent(messages: BedrockMessages): BedrockMessages {
    ...
    for (const message of messages) {
      if (message.role === "tool") {
        ...
        logger.info(
          ...
          "[BedrockAdapter] Analyzing tool result content",
        );
          ...
          for (const [idx, item] of message.content.entries()) {
            if (typeof item === "object" && item !== null) {
              ...
              logger.info(
                ...
                "[BedrockAdapter] Tool result array item",
              );
            }
          }
        }
    ...
    const result = messages.map((message) => {
      ...
      // Model supports images - convert MCP image blocks to Bedrock format
      const convertedContent = convertMcpImageBlocksToBedrock(message.content);
      if (!convertedContent) {
        return message;
      }
    ...
    if (toolMessagesWithImages > 0 || strippedImageCount > 0) {
      logger.info(
        ...
        "[BedrockAdapter] Processed tool messages with image content",
      );
    }
    ...
    if (config.features.browserStreamingEnabled) {
      ...
      if (sizeBeforeStrip.length !== sizeAfterStrip.length) {
        logger.info(
          ...
          "[BedrockAdapter] Stripped browser tool results from messages",
        );
      }
    }
      ...
      if (message.role === "tool") {
        ...
        if (toolName) {
          logger.debug(
            ...
            "[BedrockAdapter] toCommonFormat: found tool message",
          );
  ...
  private applyUpdates(
    ...
    const result = messages.map((message) => {
      if (message.role === "tool" && updates[message.tool_call_id]) {
        ...
        logger.debug(
          { toolCallId: message.tool_call_id },
          "[BedrockAdapter] applyUpdates: applying update to tool message",
        );
        return {
          ...message,
...
function convertMcpImageBlocksToBedrock(
  content: unknown,
): BedrockToolResultContent | null {
  ...
  const bedrockContent: BedrockToolResultContentBlock[] = [];
    ...
    if (isMcpImageBlock(item)) {
      ...
      if (shouldStripImage) {
        logger.info(
          ...
          "[BedrockAdapter] Stripping MCP image block due to size limit",
        );
        bedrockContent.push({
          type: "text",
      ...
      logger.info(
        ...
        "[BedrockAdapter] Converting MCP image block to Bedrock format",
      );
  ...
  logger.info(
    {
      totalBlocks: bedrockContent.length,
      imageBlocks: bedrockContent.filter((b) => b.type === "image_url").length,
      textBlocks: bedrockContent.filter((b) => b.type === "text").length,
    },
    "[BedrockAdapter] Converted MCP content to Bedrock format",
  );

  return bedrockContent.length > 0 ? bedrockContent : null;
}
...
function stripImageBlocksFromContent(content: unknown): string {
  ...
  if (imageCount > 0) {
    ...
    logger.info(
      { imageCount },
      "[BedrockAdapter] Stripped image blocks from tool result",
    );
  }
...
class BedrockStreamAdapter
  implements LLMStreamAdapter<BedrockStreamChunk, BedrockResponse>
{
  readonly provider = "bedrock" as const;
  ...
  processChunk(chunk: BedrockStreamChunk): ChunkProcessingResult {
  ...
  formatEndSSE(): string {
    const finalChunk: BedrockStreamChunk = {
      id: this.state.responseId,
      object: "chat.completion.chunk",
      created: Math.floor(Date.now() / 1000),
      model: this.state.model,
      ...
      choices: [
        {
          ...
          logprobs: null,
          finish_reason:
            (this.state.stopReason as Bedrock.Types.FinishReason) ?? "stop",
        },
      ],
      usage: {
...
async function convertToolResultsToToon(
  ...
  // Use tiktoken tokenizer (same as OpenAI) for Bedrock
  const tokenizer = getTokenizer("bedrock");
...
> = {
  provider: "bedrock",
  interactionType: "bedrock:chatCompletions",
  ...
  getBaseUrl(): string | undefined {
    return config.llm.bedrock.baseUrl;
  },

  getSpanName(_streaming: boolean): string {
    return "bedrock.chat.completions";
  },
  ...
  createClient(
    ...
    const customFetch = options?.agent
      ? getObservableFetch("bedrock", options.agent, options.externalAgentId)
      : undefined;

    // Use OpenAI SDK with Bedrock base URL (OpenAI-compatible API)
  ...
  async execute(
    ...
    const bedrockClient = client as OpenAIProvider;
    ...
    return {
      [Symbol.asyncIterator]: async function* () {
        for await (const chunk of stream) {
          yield chunk as BedrockStreamChunk;
        }
      },
    };
  },
  ...
  extractErrorMessage(error: unknown): string {
    // OpenAI SDK error structure (same for Bedrock)
